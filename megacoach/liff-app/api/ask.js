...
ปรัชญาผู้ใช้ (กำหนดทุกคำแนะนำ): กลัวพอร์ตแตกมากกว่ากลัวตกรถ (Fear of Ruin > FOMO) — เอาตัวรอดก่อน รวยทีหลัง · ขึ้นรถตอนมีจังหวะ ไม่ไล่ราคา new high แบบไม่มีแผน · ทุกไม้เก็งกำไรต้องมีจุด stop/ออกก่อนเข้า (aggressive แบบมีวินัย ไม่ใช่ YOLO) · เน้นจังหวะเข้า-ออกที่ถูกต้อง + ปกป้อง downside · ถ้าผู้ใช้กำลังไล่ของร้อน/เข้าแบบไม่มีแผน ให้เตือนตรงๆ และเสนอจังหวะ/เงื่อนไขที่ดีกว่า
...
// ลบ inline citation marker ของ web_search ที่หลุดมาใน text (อยู่ใน Private Use Area
// เช่น citeturn0finance0) ให้คำตอบสะอาด
function stripCitations(s) {
  return s
    .replace(new RegExp("[\\uE000-\\uF8FF]","g"), "")  // ลบ marker PUA ทั้งหมด
    .replace(/cite\w*turn\d+\w+/gi, "")                 // ลบ token ที่เหลือ เช่น citeturn0finance0
    .replace(/[ \t]+\n/g, "\n")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
}
